...
func RegisterCertHTTPServer(s *http.Server, srv CertHTTPServer) {
	...
	r.GET("/v1/cert/pkcs12/{ca_common}/{operate}/{gen_key_request.common}", _Cert_PKCS120_HTTP_Handler(srv))
	r.POST("/v1/cert/pkcs12", _Cert_PKCS121_HTTP_Handler(srv))
}
...
func _Cert_PKCS121_HTTP_Handler(srv CertHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in PKCS12Request
		if err := ctx.Bind(&in); err != nil {
			return err
